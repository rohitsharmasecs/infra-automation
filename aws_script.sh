...
EC2_ID2=$(aws ec2 run-instances --image-id $AMI_ID --count 1 --instance-type t2.micro --key-name pro-key-pair --security-group-ids $SG_ID --subnet-id $SUBNET_PRIVATE_ID --user-data file://Docker.sh)
...
EC2_ID1=$(aws ec2 run-instances --image-id $AMI_ID --count 1 --instance-type t2.micro --key-name pro-key-pair --security-group-ids $SG_ID --subnet-id $SUBNET_PUBLIC_ID --user-data file://Docker.sh)
